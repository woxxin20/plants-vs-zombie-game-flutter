...
| `TASK-046` | `PRD-FR-010`, `PRD-FR-011`, `PRD-FR-015`; `AUD-024` | `lib/game/worlds/battle_world.dart`, `test/ph02_exit_gate_test.dart` | Pause/Win/Lose overlays mount and accept their primary action while the battle simulation is frozen; production code never depends on a later `resumeEngine()` to flush the overlay add queue | Regression test that omits `_flushLifecycle`'s engine resume; `flutter test test/ph02_exit_gate_test.dart`; repeat pause and terminal transitions on SM-S711B | Ready |
