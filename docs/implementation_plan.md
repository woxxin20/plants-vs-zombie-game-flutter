...
| `TASK-011` | N/A (infra, closes `AUD-002`) | `assets/fonts/*.ttf`, `pubspec.yaml` `fonts:` section, `lib/core/theme.dart` | Orbitron/Inter/JetBrainsMono TTFs sourced under a license compatible with redistribution, bundled as assets, referenced via `fonts:` in `pubspec.yaml`; zero `google_fonts` runtime network calls remain in `lib/` | `grep -r "GoogleFonts\." lib/` returns 0 matches; `flutter pub deps` no longer lists `google_fonts` as used | Done |
...
| `TASK-046` | `PRD-FR-010`, `PRD-FR-011`, `PRD-FR-015`; `AUD-024` | `lib/game/worlds/battle_world.dart`, `lib/app.dart`, `test/aud024_overlay_mount_test.dart` | Pause/Win/Lose overlays mount and accept their primary action while the battle simulation is frozen; production code never depends on a later `resumeEngine()` to flush the overlay add queue | Regression test that omits `_flushLifecycle`'s engine resume; `flutter test test/ph02_exit_gate_test.dart`; repeat pause and terminal transitions on SM-S711B | Done |
| `TASK-047` | `PRD-FR-020`; `AUD-028` | `lib/game/components/hud/right_panel_component.dart`, `lib/game/components/hud/top_bar_component.dart`, `lib/game/worlds/battle_world.dart`, `test/aud028_investigation_test.dart` | RightPanel HUD is anchored to `camera.viewport.size.x - S.rightPanelW` and TopBar stretches to `camera.viewport.size.x` across non-baseline viewports without overlapping board or clipping off-screen | `flutter test test/aud028_investigation_test.dart` (assert zero board overlap on wider/baseline/narrower aspect ratios) | Done |
...
| `TASK-038` | `PH-04` | `PRD-FR-021` | `lib/core/audio.dart`, `assets/audio/`, haptic gate | All 8 `assets/audio/*.mp3` + `bgm.mp3` preloaded via `FlameAudio.audioCache.loadAll`; haptics fire per spec §23 table when `save.haptics==true` | `flutter test test/ph04_exit_gate_test.dart` passes (all 6 tests) | Done |
...
| `TASK-040` | `PH-05` | `PRD-FR-013` | AdMob console (manual), `lib/core/ads.dart` | Real (non-test) AdMob ad unit ids created for banner/interstitial/rewarded, wired via `google_mobile_ads`, closes `AUD-006` | Manual console screenshot + `grep` confirms no `ca-app-pub-3940256099942544` (Google test id) remains in release config | Withdrawn (v1) |
| `TASK-041` | `PH-05` | `PRD-FR-014` | Play Console / App Store Connect (manual), `lib/core/iap.dart` | `remove_ads` $2.99 non-consumable product id registered in both stores, wired via `in_app_purchase`, closes `AUD-006` | Manual sandbox purchase test | Withdrawn (v1) |
...
| 2026-09-09 (c16) | `TASK-047` | Added PH-02/PH-04 HUD viewport anchoring task to resolve AUD-028 after investigation proved panel position hardcoded to baseline 812x375 causing non-baseline overlap/clipping | `AUD-028`; `test/aud028_investigation_test.dart` evidence | GameDesigner (lead) |
| 2026-09-07 (c10) | `TASK-046` | Added PH-02 overlay-ordering remediation after hardware showed Pause/Win/Lose freezing before their viewport overlays mount | `AUD-024`; SM-S711B device evidence | GameDesigner (lead) |
