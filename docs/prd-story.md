...
Status: Active · Owner: repo owner · Last verified 2026-09-07 (c14) against `main` @ `82ed6ab` + uncommitted c13/c14 tree
...
| `A-3` | Resolve `AUD-027` | `startBgm()` has a production call site, or BGM is struck from `PRD-FR-021` | **Human** (owner call) |
...
| 3 | Does BGM stay in scope? `startBgm()` is dead code and `bgm.mp3` is 321KB of never-played payload (`AUD-027`) | `PH-04` exit, `A-3`, `PRD-FR-021` |
